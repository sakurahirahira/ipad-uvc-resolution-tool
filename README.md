...
- **縦型（ポートレート）解像度は非対応**: 安価なHDMI-to-USB UVCキャプチャカードは内部スケーラーの制限により、縦型解像度（高さ＞幅）をサポートしていません。EDID Override等のレジストリ操作でも解決不可。縦型表示が必要な場合は、横型解像度を設定後にWindows側で画面回転（90度）を使用してください。
